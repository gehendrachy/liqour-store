...
class SalesReportController extends Controller
{
    ...
    {
        $sales_data = $this->create_update_sales_report($request);
        ...
        $sales_reports = SalesReport::with('vendor.vendor_orders.ordered_products')->where([['report_date', '>=', $report_start_from],['report_date', '<=', $report_end_to]]);
        ...
        $sales_reports = $sales_reports->get()->pluck('vendor.vendor_orders')->flatten()->groupBy('vendor_id')->all(); 
    ...
    public function create_update_sales_report($request)
    {
        ...
        do
        {
            ...
            foreach ($vendor_with_orders as $vendor_id => $single_vendor_orders) {
                ...
                $single_vendor_order_all_ordered_products = $single_vendor_orders->pluck('ordered_products')->flatten();

                $vendor = User::with('vendor_details')->where('id',$vendor_id)->first();

                if (array_key_exists($vendor_id, $sales_data)) {
                    echo $vendor_id.'a';
                    $sales_data[$vendor_id]['order_count'] += count($single_vendor_orders);
                }else{
                    echo $vendor_id.'b';
                    $sales_data[$vendor_id]['vendor_name'] = $vendor->vendor_details->store_name;
                    $sales_data[$vendor_id]['order_count'] = count($single_vendor_orders);
                }
                ...
                $return_refund = $single_vendor_order_all_ordered_products->where('status', 6)->sum('grand_total');
