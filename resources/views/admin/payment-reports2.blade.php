                                    ...
                                    <div style="width: 100%;" class="btn-group" role="group">
                                        ...
                                        <div class="dropdown-menu" style="width: 100%;">
                                            ...
                                            @foreach($vendors as $key => $vendor)
                                                <button onclick="filter_by_vendor({{ $vendor->user_id }})" class="btn btn-info dropdown-item sales-report-status-btn" data-vendor-id="{{ $vendor->id }}" href="">{{ $vendor->store_name }}</button>
                                            @endforeach
                                        </div>
                                    </div>
                                </div>    
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-hover table-custom spacing5 dataTable payment-table">
                                            ...
                                            <tbody id="product_variations_field">
                                                ...
                                                <tr class="vendor_payments vendor_{{ $payment_report->vendor_id }}">
                                                    <td style="font-size: 12px;">{{ $payment_report->payment_id }}</td>
                                                    <td style="font-size: 10px;">{{ $payment_report->vendor->vendor_details->store_name }}</td>
                                                    <td>
                                                        @php
                                                    ...
                                                    <td class="text-right">
                                                        $<span class="vendor_{{ $payment_report->vendor_id }} total_amount">{{ number_format($payment_report->total_amount, 2, '.', '') }}</span>
                                                    </td>
                                                    ...
                                                    {{-- <td class="text-right">${{ $payment_report->paid_by }}</td> --}}
                                                    {{-- <td>
                                                        <div class="btn-group" role="group">
                                                            ...
                                                            <div class="dropdown-menu">
                                                                @for($i = 0; $i < count($paymentStatus); $i++)
                                                                <button class="btn btn-info dropdown-item payment-report-status-btn" data-payment-report-id="{{ $payment_report->id }}" data-status="{{ $i }}" href="">{{ $paymentStatus[$i][0] }}</button>
                                                                @endfor
                                                            </div>
                                                        </div>

                                                        <a href="{{ route('admin.payment-reports.edit',['id' => base64_encode($payment_report->id)]) }}" class="btn btn-sm btn-outline-primary" title="Edit"><i class="fa fa-edit"></i></a>
...
<script type="text/javascript">
    ...
    function filter_by_vendor(vendor_id) {
        $(".vendor_payments").each(function(){
            if ($(this).hasClass('vendor_'+vendor_id)) {
                $(this).show();
            }else{
                $(this).hide();
            }
        });
        calculate_total_prices(vendor_id);
        // total = 0;
        // $('.vendor_'+vendor_id +' .total_amount').each(function(){
        //     // console.log($(this).text());
        //     total += parseFloat($(this).text());
        // });

        // $('#totalAmountSum').html(total.toFixed(2));
    }

    function calculate_total_prices(vendor_id = 0) {
        ...
        for (var i = 0; i < field_array.length; i++) {
            ...
            if (vendor_id == 0) {
                var vendor_field_class = '.'+field_array[i]
            }else{
                var vendor_field_class = '.vendor_'+vendor_id+' .'+field_array[i]
            }

            $(vendor_field_class).each(function(){
                // console.log($(this).text());
