...
<body class="common-home res layout-home2 banners-effect-7">
                            ...
                            <div class="header-top-right collapsed-block text-right  col-sm-6 col-xs-6 hidden-xs">
                                ...
                                <div class="tabBlock" id="TabBlock-1">
                                    <ul class="top-link list-inline">
                                        <li class="my-accout ">
                                            <a href="{{ route('customer.my-account') }}" id="wishlist-total" class="top-link-wishlist" title="My Account"><span>My Account</span></a>
                                        </li>
                                        <li class="wishlist">
                                            <a href="{{ route('customer.wishlist') }}" title="Wishlist"><span>Wishlist</span></a>
                                        </li>
                                        @if(Auth::check())
                                        <li class="checkout">
                                            <a  class="top-link-checkout" title="Log Out" href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();"><span>Log Out</span></a>
                            ...
                            <div class="col-lg-3 col-md-4 col-sm-5 col-xs-12 shopping_cart">
                                ...
                                <div id="cart" class=" btn-group btn-shopping-cart">
                                    ...
                                    <ul class="tab-content content dropdown-menu pull-right shoppingcart-box" role="menu" id="cartQuickView">
                                        ...
                                        @endphp
                                        @if($count > 0)
                                            <li style="overflow-y: scroll; max-height: 350px;">
                                                        ...
                                                        @foreach($items as $i => $item)
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td class="text-left">
                                                                    ...
                                                                    <p>${{$item['cart_subTotal']/$item['cart_orderedQty']}} x {{$item['cart_orderedQty']}} </p>
                                                                    <a>{{ $invProd->store->vendor_details->store_name }}</a>
                                                                </td>
                                                                <td class="text-center"><strong> ${{$item['cart_subTotal']}} </strong> </td>
                                                                <td class="text-right">
                                    ...
                                    <div class="link">
                                        @if(Auth::check())
                                            @if (Auth::user()->hasRole(['Vendor'])) 
                                                ...
                                                <a href="{{ route('vendor.dashboard',['username' => Auth::user()->username]) }}">Dashboard</a>
                                                    ...
                                                    <div class="container">
                                                        <ul class="megamenu " data-transition="slide" data-animationtime="250">
                                                            ...
                                                            <li class="with-sub-menu hover">
                                                                ...
                                                                <a href="{{ route('parent_category_products',['slug' => $category->slug]) }}" class="clearfix menu1">
    ...
    <script type="text/javascript">
        function addToWishlist(productId, productSlug, productName, productImage) {

            wishlistAdd(productId, productSlug, productName, productImage);
        }

        function wishlistAdd(productId, productSlug, productName, productImage) {

            $.ajax({
                url : "{{ URL::route('customer.add-to-wishlist') }}",
                type : "POST",
                ...
                success : function(response)
                {
                    ...
                    if (obj.status=='success') {
                        ...
                        addProductNotice('Product added to Wishlist', 
                            ...
                            '<h3><a href="{{ url("product/")}}/'+productSlug+'">'+productName+'</a> added to <a href="{{ route("customer.wishlist") }}">Wishlist</a>!</h3>', 'success');

                    }else if (obj.status=='exist') {

                        addProductNotice('Product romoved from Wishlist', 
                            '<img src="'+ productImage +'" alt="'+productName+'">', 
                            '<h3><a href="{{ url("product/")}}/'+productSlug+'">'+productName+'</a> removed from <a href="{{ route("customer.wishlist") }}">Wishlist</a>!</h3>', 'success');

                        $('#wishlistItems').load(document.URL + ' #wishlistItems>*');

                    }else if (obj.status=='error') {

                        addProductNotice('Something went wrong!', 
                            '<img src="'+ productImage +'" alt="'+productName+'">', 
                            '<h3><a href="{{ url("product/")}}/'+productSlug+'">'+productName+'</a> could not be added to <a href="{{ route("customer.wishlist") }}">Wishlist</a>!</h3>', 'error');

                    }else if(obj.status == 'login-error') {
                        
                        addProductNotice('Please Login First!', 
                            '<img src="'+productImage+'" alt="'+productName+'">',
                            '<h3>You must <a href="{{ url("user/login") }}">login</a>  to save <a href="{{ url("product/")}}/'+productSlug+'">'+productName+'</a> to your <a href="{{ route("customer.wishlist") }}">wish list</a>!</h3>', 'error');

                    }else if(obj.status == 'not-a-customer') {
                        addProductNotice('Permission Error!', '',
                            '<h3>You must be logged in as a <strong>customer</strong>!</h3>', 'error');
                    };
                }
            });
        }
            ...
            $.ajax({
                ...
                success : function(response)
                {
                    ...
                    if (obj.status=='success') {
                        ...
                        call_ajax_function(productVariationId);
                        ...
                        addProductNotice('Oops! Out of Stock', 
                            '<img src="'+productImage+'" alt="'+productName+'">', 
                            '<h3><a href="{{ url("product/")}}/'+productSlug+'">'+productName+'</a> <br>Available Stock: '+stock+'.<br>In Cart: '+in_cart+'.</h3>', 'error');
                    };
                }
            });
        }
        ...
        $(".btn-add-to-wishlist").click(function(){
            ...
            addToWishlist(productId, productSlug, productName, productImage);
